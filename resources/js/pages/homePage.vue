...
<template>
    <Layout>
        <main ref="pageRoot" class="">
            <div class="min-h-[calc(100vh-170px)] flex flex-col justify-between h-full">
                <div class="relative z-40 flex  flex-col gap-4 px-2.5 pt-4">
                    <img src="/assets/img/emerald-rotate.png" class="absolute top-1/2 -right-5 z-10 h-11 -translate-y-1/2 -scale-x-100" alt="" data-animate-image />
                    <div class="flex flex-col gap-1">
                        <Swiper class="w-full" :modules="[Autoplay]" :slides-per-view="1" :loop="true" :speed="650" :autoplay="{
                            ...
                            <SwiperSlide v-for="s in slides" :key="s.key" class="w-full">
                                <div class="relative flex h-44 flex-col justify-between overflow-hidden rounded-[10px] p-5">
                                    ...
                                    <div class="relative z-10 flex flex-col gap-2.5">
                                        <h1 class="font-minecraft-ten text-sm uppercase" :style="{ color: s.titleColor }">
                                            {{ s.title }}
                                        </h1>
                                        <h2 class="max-w-[145px] text-[10px] text-white">
                                            {{ s.subtitle }}
                                        </h2>
                                    </div>
                        ...
                        <div class="flex w-full gap-1" data-animate>
                            <div v-for="it in indicators" :key="it.i" class="relative h-1 w-full cursor-pointer overflow-hidden rounded-3xl bg-[#333333]" role="button" tabindex="0" @click="goTo(it.i)">
                                <div class="absolute top-0 left-0 h-full w-full origin-left rounded-3xl bg-green-main transition-transform duration-150 ease-out" :style="{ transform: `scaleX(${it.progress})` }"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-4" data-animate>
                        ...
                        <div class="flex max-w-[350px] gap-2.5 overflow-x-auto">
                            <Link v-for="game in games" :key="game.id" :href="`/${game.url_slug}`" class="group relative h-[144px] min-w-[110px] cursor-pointer overflow-hidden rounded-[5px] duration-300 ease-in-out">
                                <div class="absolute top-0 left-0 z-20 flex h-full w-full items-center justify-center bg-black/85 opacity-0 duration-300 ease-in-out group-hover:opacity-100">
                                    <svg width="50" height="53" viewBox="0 0 50 53" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="25" cy="28" r="25" fill="#4F7632" />
                                        <circle cx="25" cy="25" r="25" fill="#6CA243" />
                                        <path d="M35.8761 23.5728C36.4454 23.8533 36.9277 24.2834 37.2713 24.817C37.6148 25.3506 37.8067 25.9676 37.8264 26.602C37.8461 27.2363 37.6928 27.864 37.383 28.4179C37.0732 28.9718 36.6185 29.431 36.0677 29.7462L21.3796 38.3341C19.0146 39.7183 16.0069 38.1075 15.9233 35.4149L15.419 19.1652C15.3354 16.4715 18.2376 14.6785 20.6838 15.911L35.8761 23.5728Z" fill="#AFAFAF" />
                                        <path d="M35.8761 21.5728C36.4454 21.8533 36.9277 22.2834 37.2713 22.817C37.6148 23.3506 37.8067 23.9676 37.8264 24.602C37.8461 25.2363 37.6928 25.864 37.383 26.4179C37.0732 26.9718 36.6185 27.431 36.0677 27.7462L21.3796 36.3341C19.0146 37.7183 16.0069 36.1075 15.9233 33.4149L15.419 17.1652C15.3354 14.4715 18.2376 12.6785 20.6838 13.911L35.8761 21.5728Z" fill="white" />
                                    </svg>
                                </div>
                                <img :src="'/storage/' + game.image" class="absolute bottom-0 left-0 h-full w-full" alt="" />
                            </Link>
                ...
                <div class="flex text-[10px] px-2.5 justify-between text-white/60">
                    <a href="https://telegra.ph/Politika-konfidencialnosti-Mine-Drop-12-28" target="_blank" rel="noopener noreferrer">Политика конфиденциальности</a>
                    <a href="https://telegra.ph/Polzovatelskoe-soglashenie-MineDrop-12-19" target="_blank" rel="noopener noreferrer">Условия использования</a>
                </div>
            </div>
